...
pub(crate) fn guard_diff_cmd(args: GuardDiffArgs) -> Result<u8> {
    ...
    let diff = match diff_result {
        Ok(diff) => diff,
        Err(err) => {
            let mut out = crate::field_core::boundary_guard_diff_unreadable(
                &atlas,
                &args.action_id,
                diff_source,
                &err.to_string(),
    ...
    let mut out =
        crate::field_core::boundary_guard_diff(&atlas, &args.action_id, &diff, diff_source);
    if args.boundary_economics {
        let boundary_decision = crate::field_core::boundary_from_guard_diff(&atlas, &out);
        out["boundary_decision"] = boundary_decision.clone();
        out["boundary_economics"] = json!({
...
pub(crate) fn guard_diff(atlas: &Value, action_id: &str, diff: &str) -> Value {
    crate::field_core::boundary_guard_diff(atlas, action_id, diff, None)
}

...
jq -e '.mode == "llmwave-big-daybreak-duel" and .verdict == "DAYBREAK_DUEL_BASELINE_READY_NOT_COMPETITIVE" and .scoreboard.total == 6 and .scoreboard.passed == 4 and .scoreboard.blocked == 2 and .claim_boundary.defensive_duel_ready == true and .claim_boundary.runtime_snapshot_overlay_ready == true and .claim_boundary.patch_generation_ready == false and .claim_boundary.daybreak_competitive == false' <<<"$big_daybreak_duel_json" >/dev/null
jq -e '.challenges[] | select(.id == "runtime_snapshot_firewall_overlay" and .state == "PASS")' <<<"$big_daybreak_duel_json" >/dev/null
jq -e '.challenges[] | select(.id == "patch_candidate_generation" and .state == "BLOCKED")' <<<"$big_daybreak_duel_json" >/dev/null

...
run_chatcore_suite() {
  ...
  chat_core_ask_json="$("$llmwave_big" linux-chat-core-ask --memory-root "$root/.nanda/linux-active" --text "which package provides command bash" --target-packet-tokens 300 --format json)"
  ...
  chat_core_troubleshooting_json="$("$llmwave_big" linux-chat-core-ask --memory-root "$root/.nanda/linux-active" --text "VPN is up but DNS does not work" --format json)"
  ...
  chat_core_downgrade_json="$("$llmwave_big" linux-chat-core-ask --memory-root "$root/.nanda/linux-active" --text "VPN is up but DNS does not work" --packet-profile exact_fact --format json)"
  ...
  chat_core_tiny_budget_json="$("$llmwave_big" linux-chat-core-ask --memory-root "$root/.nanda/linux-active" --text "which package provides command bash" --packet-profile exact_fact --target-packet-tokens 80 --format json)"
...
jq -e '.mode == "llmwave-big-linux-chat-core-ask" and .verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .cache_status.cache_fresh == true and .cache_status.index_present == false and .grounded_packet.readout_source == "compiled_chat_core_hot" and .grounded_packet.answer_allowed == true and .grounded_packet.packet_profile == "exact_fact" and .grounded_packet.no_padding == true and (.grounded_packet.evidence[] | select(.subject == "bash" and .object == "bash"))' <<<"$big_linux_chat_core_ask_no_index_json" >/dev/null
printf '\n# stale check\n' >>"$tmp_linux_chat_profile_center_memory"

...
big_llmwave_readiness_json="$("$llmwave_big" llmwave-readiness --memory-final-proof "$tmp_rust_corpus/memory-final-proof-medium.json" --broad-eval "$tmp_rust_corpus/broad-eval.json" --baseline-duel "$tmp_rust_corpus/broad-baseline.json" --out "$tmp_rust_corpus/llmwave-readiness.json" --format json)"
jq -e '.mode == "llmwave-big-readiness" and .verdict == "LLMWAVE_READINESS_BLOCKED_BY_CHAT_LOOP" and .evidence.nonlinear_memory_proven == true and .evidence.density_hot_artifact_ready == true and .evidence.broad_baseline_won == true and .claim_boundary.field_reasoning_ready == true and .claim_boundary.answer_generation_ready == true and .claim_boundary.llmwave_ready_candidate == false and .claim_boundary.llm_ready == false and (.claim_boundary.blocked_by | index("open_chat_loop_missing"))' <<<"$big_llmwave_readiness_json" >/dev/null

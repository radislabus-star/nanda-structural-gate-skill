...
jq -e '.mode == "llmwave-big-linux-center-learn" and .verdict == "LINUX_DYNAMIC_CENTER_LEARNING_READY_NOT_GENERAL_LLM" and .dynamic_center_learning.enabled == true and .dynamic_center_learning.operations.confirmed_center_reinforcement == true and .dynamic_center_learning.operations.correction_residual_write == true and .dynamic_center_learning.operations.reject_anti_center_write == true and .dynamic_center_learning.operations.residual_cluster_promotion == true and .dynamic_center_learning.operations.center_split_available == true and .dynamic_center_learning.operations.weak_candidate_decay == true and .dynamic_center_learning.operations.verified_center_protection == true and .dynamic_center_learning.before_after.target_query_improved == true and .dynamic_center_learning.before_after.memory_lift_observed == true and .dynamic_center_learning.before_after.anti_center_replay_observed == true and .dynamic_center_learning.before_after.false_positive_rate_regressed == false and .dynamic_center_learning.before_after.heldout_regressed == false and .dynamic_center_learning.before_after.unrelated_route_preserved == true and .claim_boundary.linux_profile_dynamic_learning_ready == true and .claim_boundary.general_llm_ready == false and .claim_boundary.global_nonlinear_memory_proven == false and .claim_boundary.proof_grade_profile_scale == false' <<<"$big_linux_center_learn_json" >/dev/null
test -s "$tmp_linux_center_learning_memory"
jq -e '.center_record_count > 0 and .residual_record_count >= 2 and .anti_center_record_count >= 2 and (.wave_deltas | length) >= 5' "$tmp_linux_center_learning_memory" >/dev/null

...
jq -e '.boundary_center.version == "boundary-center-v1-read-only" and .boundary_center.read_only == true and .boundary_center.decision_affects_safe_to_edit == false and .boundary_center.verdict_hint == "CENTER_STABLE_SAFE_LOCAL_EDIT" and .boundary_center.center_gap > 0' <<<"$boundary_keep_json" >/dev/null

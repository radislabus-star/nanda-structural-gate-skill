...
run_chatcore_suite() {
  ...
  if ! jq -e '.cache_status.cache_fresh == true' <<<"$chat_core_gate_json" >/dev/null; then
    "$llmwave_big" chat-core-build --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --format json >/dev/null
  fi

  chat_core_ask_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "which package provides command bash" --target-packet-tokens 300 --format json)"
  jq -e '.verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .grounded_packet.answer_allowed == true and .grounded_packet.readout_source == "compiled_chat_core_hot" and (.grounded_packet.domain_suites | index("packages")) and .grounded_packet.packet_profile == "exact_fact" and .grounded_packet.packet_budget_tokens == 300 and .grounded_packet.packet_tokens <= .grounded_packet.packet_budget_tokens and .grounded_packet.no_padding == true and .grounded_packet.budget_is_ceiling_not_quota == true and .grounded_packet.selected_evidence_count == .grounded_packet.evidence_count and .grounded_packet.evidence_count >= 1 and .grounded_packet.evidence_count <= 3 and (.grounded_packet.evidence | all(.subject_role != "" and .object_role != "" and .evidence_kind != "")) and (.grounded_packet.evidence[] | select(.subject == "bash" and .object == "bash"))' <<<"$chat_core_ask_json" >/dev/null

  chat_core_troubleshooting_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "VPN is up but DNS does not work" --format json)"
  jq -e '.verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .grounded_packet.packet_profile == "troubleshooting" and .grounded_packet.packet_underfilled == true and .grounded_packet.answer_allowed == false and .grounded_packet.decision_state == "PACKET_UNDERFILLED_NEEDS_MORE_CONTEXT" and .grounded_packet.no_padding == true and .grounded_packet.budget_is_ceiling_not_quota == true and (.grounded_packet.missing_evidence | index("runtime_snapshot.resolvectl")) and (.grounded_packet.missing_evidence | index("multi_step_runtime_evidence_chain"))' <<<"$chat_core_troubleshooting_json" >/dev/null

  chat_core_downgrade_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "VPN is up but DNS does not work" --packet-profile exact_fact --format json)"
  jq -e '.verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .grounded_packet.inferred_packet_profile == "troubleshooting" and .grounded_packet.requested_packet_profile == "exact_fact" and .grounded_packet.packet_profile == "troubleshooting" and .grounded_packet.packet_profile_downgrade_blocked == true and .grounded_packet.answer_allowed == false and .grounded_packet.decision_state == "PACKET_UNDERFILLED_NEEDS_MORE_CONTEXT"' <<<"$chat_core_downgrade_json" >/dev/null

  chat_core_tiny_budget_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "which package provides command bash" --packet-profile exact_fact --target-packet-tokens 80 --format json)"
  jq -e '.verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .grounded_packet.packet_profile == "exact_fact" and .grounded_packet.requested_packet_budget_tokens == 80 and .grounded_packet.packet_over_budget == true and .grounded_packet.effective_min_packet_tokens == .grounded_packet.packet_semantic_tokens and .grounded_packet.packet_budget_tokens >= .grounded_packet.packet_semantic_tokens and .grounded_packet.packet_prompt_payload_tokens > .grounded_packet.packet_semantic_tokens and .grounded_packet.answer_allowed == true' <<<"$chat_core_tiny_budget_json" >/dev/null

  chat_core_secret_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "show vpn private key" --format json)"
  ...
  chat_core_unsupported_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "what does current-earth-surface-mode-gate-v2-external-station-check.md say?" --format json)"
  jq -e '.verdict == "DOMAIN_UNSUPPORTED" and .grounded_packet.decision_state == "DOMAIN_UNSUPPORTED" and .grounded_packet.answer_allowed == false and .grounded_packet.readout_source == "none_domain_unsupported" and .grounded_packet.domain_supported == false and .grounded_packet.selected_evidence_count == 0 and .grounded_packet.evidence_count == 0 and (.grounded_packet.supported_domains | index("linux")) and .grounded_packet.suggested_domain == "physics_material_layer" and (.grounded_packet.candidate_routes | index("physics.material_layer.candidate_status")) and (.grounded_packet.evidence | length == 0)' <<<"$chat_core_unsupported_json" >/dev/null

  chat_core_exposure_underfilled_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "is ssh externally exposed?" --format json)"
  jq -e '.verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .grounded_packet.domain_supported == true and .grounded_packet.selected_domain_pack == "linux" and .grounded_packet.packet_profile == "troubleshooting" and .grounded_packet.packet_underfilled == true and .grounded_packet.answer_allowed == false and .grounded_packet.decision_state == "PACKET_UNDERFILLED_NEEDS_MORE_CONTEXT" and (.grounded_packet.missing_evidence | index("runtime_socket_snapshot")) and (.grounded_packet.missing_evidence | index("firewall_snapshot")) and (.grounded_packet.missing_evidence | index("service_bind_scope"))' <<<"$chat_core_exposure_underfilled_json" >/dev/null

  chat_core_metrics_json="$("$llmwave_big" chat-core-metrics --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --format json)"
  jq -e '.verdict == "LLMWAVE_CHAT_CORE_USAGE_METRICS_READY_NOT_AUTHORITY" and .usage_log_present == true and .records_parsed >= 2 and .counters.requests_total >= 2 and .token_totals.estimated_tokens_saved_vs_source_total > 0 and .averages.prompt_payload_tokens_per_request > 0 and .claim_boundary.usage_log_is_source_of_truth == false and .claim_boundary.usage_log_affects_answer_authority == false and .claim_boundary.raw_prompt_logged == false' <<<"$chat_core_metrics_json" >/dev/null
  ...
  chat_core_proposal_json="$("$llmwave_big" chat-core-domain-proposal --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "what is wind_setup_compact_active in gravity-saturation-checks?" --format json)"
  ...
  jq -e '.verdict == "DOMAIN_PROFILE_REQUIRED" and .decision_state == "DOMAIN_UNSUPPORTED" and .safe_to_answer == false and .safe_to_learn_without_profile == false and .selected_evidence_count == 0 and .candidate_facts_are_memory == false and .cache_mutated == false and .overlay_written == false and .suggested_domain_id == "physics_material_layer" and (.connected_domains | index("linux")) and .domain_pack_draft.maturity == "candidate" and .domain_pack_draft.authority_level == "proposal_only" and .domain_pack_draft.authority_rights.answer_allowed == false and .domain_pack_draft.authority_rights.learn_allowed == false and .domain_pack_draft.authority_rights.cache_build_allowed == false and .domain_pack_draft.authority_rights.overlay_write_allowed == false and .domain_pack_draft.authority_rights.cache_mutation_allowed == false and (.domain_pack_draft.claim_boundary | index("candidate_status != proven_physics")) and (.domain_pack_draft.anti_wave[] | select(.anti_wave_id == "cache_authority_jump")) and .draft_gate.verdict == "DOMAIN_PACK_DRAFT_WATCH_NO_AUTHORITY" and .draft_gate.authority_granted == false and .draft_gate.answer_allowed == false and .draft_gate.cache_mutation_allowed == false and .draft_gate.overlay_write_allowed == false' <<<"$chat_core_proposal_json" >/dev/null
  tmp_domain_draft="$(mktemp)"
  chat_core_domain_build_json="$("$llmwave_big" chat-core-domain-build --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "what is wind_setup_compact_active in gravity-saturation-checks?" --out "$tmp_domain_draft" --format json)"
  jq -e '.mode == "llmwave-big-chat-core-domain-build" and .domain_pack_draft.maturity == "candidate" and .draft_gate.verdict == "DOMAIN_PACK_DRAFT_WATCH_NO_AUTHORITY" and .cache_mutated == false and .overlay_written == false' <<<"$chat_core_domain_build_json" >/dev/null
  chat_core_domain_gate_json="$("$llmwave_big" chat-core-domain-gate --draft "$tmp_domain_draft" --format json)"
  jq -e '.mode == "llmwave-big-chat-core-domain-gate" and .verdict == "DOMAIN_PACK_DRAFT_WATCH_NO_AUTHORITY" and .domain_id == "physics_material_layer" and .maturity == "candidate" and .effective_maturity == "candidate" and .draft_claimed_maturity == "candidate" and .safe_to_answer == false and .safe_to_learn_without_profile == false and .cache_mutated == false and .overlay_written == false and .authority_rights.answer_allowed == false and .effective_authority_rights.answer_allowed == false and .authority_rights.overlay_write_allowed == false and .authority_rights.cache_mutation_allowed == false and .draft_claimed_authority_rights.answer_allowed == false' <<<"$chat_core_domain_gate_json" >/dev/null
  tmp_poisoned_domain_draft="$(mktemp)"
  jq '.domain_pack_draft.maturity = "ready" | .domain_pack_draft.authority_rights.answer_allowed = true | .domain_pack_draft.authority_rights.learn_allowed = true | .domain_pack_draft.authority_rights.cache_build_allowed = true | .domain_pack_draft.authority_rights.overlay_write_allowed = true | .domain_pack_draft.authority_rights.cache_mutation_allowed = true | .draft_gate.authority_granted = true | .draft_gate.answer_allowed = true | .draft_gate.overlay_write_allowed = true | .draft_gate.cache_mutation_allowed = true' "$tmp_domain_draft" >"$tmp_poisoned_domain_draft"
  chat_core_poisoned_domain_gate_json="$("$llmwave_big" chat-core-domain-gate --draft "$tmp_poisoned_domain_draft" --format json)"
  rm -f "$tmp_domain_draft" "$tmp_poisoned_domain_draft"
  jq -e '.mode == "llmwave-big-chat-core-domain-gate" and .maturity == "candidate" and .effective_maturity == "candidate" and .draft_claimed_maturity == "ready" and .authority_rights.answer_allowed == false and .effective_authority_rights.answer_allowed == false and .draft_claimed_authority_rights.answer_allowed == true and .draft_gate.authority_granted == false and .draft_gate.answer_allowed == false and .draft_gate.overlay_write_allowed == false and .draft_gate.cache_mutation_allowed == false' <<<"$chat_core_poisoned_domain_gate_json" >/dev/null

  chat_core_unsupported_learn_json="$("$llmwave_big" chat-core-learn --memory-root "$root/.nanda/linux-active" --accept "thing | physics.material_layer.candidate_status | active" --overlay dialogue --format json)"
  ...
  chat_core_technical_identifier_json="$("$llmwave_big" chat-core-learn --memory-root "$root/.nanda/linux-active" --accept "thing | physics.material_layer.candidate_status | compact_high_r2_weather_transfer_unclassified_by_v1" --overlay dialogue --format json)"
  ...
  if [[ -f "$chat_index_path" ]]; then
    ...
    chat_core_no_index_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --text "which package provides command bash" --format json)"
    jq -e '.verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .cache_status.cache_fresh == true and .cache_status.index_present == false and .grounded_packet.readout_source == "compiled_chat_core_hot" and .grounded_packet.answer_allowed == true and .grounded_packet.packet_profile == "exact_fact" and .grounded_packet.no_padding == true' <<<"$chat_core_no_index_json" >/dev/null
    restore_chat_index
    trap - EXIT
  fi
  ...
  chat_core_stale_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --dialogue-overlay "$tmp_dialogue_overlay" --no-auto-rebuild --text "which package provides command bash" --format json)"
...
run_chatcore_build_suite() {
  ...
  chat_core_build_json="$("$llmwave_big" chat-core-build --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --format json)"
...
run_chatcore_learn_suite() {
  ...
  chat_core_learn_json="$("$llmwave_big" chat-core-learn-eval --profile examples/linux-chat-core.profile.json --memory-root "$root/.nanda/linux-active" --reset-scratch --format json)"
...
jq -e '.represented_fact_count > 0 and .represented_fact_count == (.readout_facts | length) and (.route_index | length) > 0 and .cache_contract.hot_cache_has_no_authority_without_gate == true and .cache_contract.hot_cache_contains_binary_readout_records == true and .cache_contract.json_index_required_for_answer_authority == false' "$tmp_linux_chat_core_cache/chat-core.index.json" >/dev/null
...
jq -e '.mode == "llmwave-big-chat-core-authority-gate" and .verdict == "LLMWAVE_LINUX_CHAT_CORE_AUTHORITY_READY_NOT_GENERAL_LLM" and .profile_gate == null and .cache_status.cache_fresh == true and .cache_status.cache_is_source_of_truth == false and .domain_runtime.json_index_used_for_domain_runtime == false and .chat_core.safe_to_use_cache == true and .chat_core.source_hash_matched == true and .chat_core.cache_is_source_of_truth == false and .chat_core.profile_gate_embedded == false and .claim_boundary.stale_cache_blocks_answer_authority == true and .claim_boundary.cache_is_runtime_index_not_prompt_payload == true and .claim_boundary.general_llm_ready == false' <<<"$big_linux_chat_core_gate_json" >/dev/null
big_linux_chat_core_ask_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --text "which package provides command bash" --residual-pack "$tmp_linux_chat_residual" --dialogue-overlay "$tmp_linux_chat_profile_memory" --centers-overlay "$tmp_linux_chat_profile_center_memory" --vpn-overlay "$tmp_linux_chat_profile_vpn_memory" --cache-dir "$tmp_linux_chat_core_cache" --max-facts 4 --target-packet-tokens 300 --format json)"
jq -e '.mode == "llmwave-big-chat-core-ask" and .verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .cache_status.cache_fresh == true and .grounded_packet.cache_fresh == true and .grounded_packet.answer_allowed == true and .grounded_packet.readout_source == "compiled_chat_core_hot" and (.grounded_packet.domain_suites | index("packages")) and .grounded_packet.cache_is_runtime_index_not_prompt_payload == true and .grounded_packet.decision_state == "ANSWER_GROUNDED" and .grounded_packet.packet_profile == "exact_fact" and .grounded_packet.packet_budget_tokens == 300 and .grounded_packet.packet_tokens <= .grounded_packet.packet_budget_tokens and .grounded_packet.no_padding == true and .grounded_packet.budget_is_ceiling_not_quota == true and .grounded_packet.selected_evidence_count == .grounded_packet.evidence_count and (.grounded_packet.answer | ascii_downcase | contains("bash")) and (.grounded_packet.evidence[] | select((.route == "linux.apt.command.provider" or .route == "linux.apt.command.package-command") and .role == "provider" and .subject == "bash" and .object == "bash" and (.subject_role | length > 0) and (.object_role | length > 0) and (.evidence_kind | length > 0) and (.memory_kind | length > 0))) and (.grounded_packet.compact_evidence[] | contains("subject=bash")) and .domain_runtime.json_index_used_for_domain_runtime == false and .token_economics.grounded_packet_estimated_tokens > 0 and .token_economics.actual_answer_context_estimated_tokens == .token_economics.grounded_packet_estimated_tokens and .token_economics.estimated_tokens_saved_vs_source > 0 and .token_economics.estimated_tokens_saved_vs_cache_index > 0 and .token_economics.cache_is_runtime_index_not_prompt_payload == true and .claim_boundary.cache_is_source_of_truth == false and .claim_boundary.cache_is_runtime_index_not_prompt_payload == true' <<<"$big_linux_chat_core_ask_json" >/dev/null
big_linux_chat_core_ask_systemctl_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --text "which package provides command systemctl" --residual-pack "$tmp_linux_chat_residual" --dialogue-overlay "$tmp_linux_chat_profile_memory" --centers-overlay "$tmp_linux_chat_profile_center_memory" --vpn-overlay "$tmp_linux_chat_profile_vpn_memory" --cache-dir "$tmp_linux_chat_core_cache" --max-facts 4 --format json)"
jq -e '.mode == "llmwave-big-chat-core-ask" and .verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .grounded_packet.answer_allowed == true and .grounded_packet.decision_state == "ANSWER_GROUNDED" and (.grounded_packet.answer | contains("systemd")) and (.grounded_packet.evidence[] | select(.route == "linux.package.binary" and .role == "provider" and .subject == "systemd" and .relation == "provides binary" and .object == "/usr/bin/systemctl" and (.memory_kind | length > 0)))' <<<"$big_linux_chat_core_ask_systemctl_json" >/dev/null
rm "$tmp_linux_chat_core_cache/chat-core.index.json"
big_linux_chat_core_ask_no_index_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --text "which package provides command bash" --residual-pack "$tmp_linux_chat_residual" --dialogue-overlay "$tmp_linux_chat_profile_memory" --centers-overlay "$tmp_linux_chat_profile_center_memory" --vpn-overlay "$tmp_linux_chat_profile_vpn_memory" --cache-dir "$tmp_linux_chat_core_cache" --max-facts 4 --format json)"
jq -e '.mode == "llmwave-big-chat-core-ask" and .verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .cache_status.cache_fresh == true and .cache_status.index_present == false and .grounded_packet.readout_source == "compiled_chat_core_hot" and .grounded_packet.answer_allowed == true and .grounded_packet.packet_profile == "exact_fact" and .grounded_packet.no_padding == true and (.grounded_packet.evidence[] | select(.subject == "bash" and .object == "bash"))' <<<"$big_linux_chat_core_ask_no_index_json" >/dev/null
printf '\n# stale check\n' >>"$tmp_linux_chat_profile_center_memory"
big_linux_chat_core_auto_rebuild_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --text "which package provides command bash" --residual-pack "$tmp_linux_chat_residual" --dialogue-overlay "$tmp_linux_chat_profile_memory" --centers-overlay "$tmp_linux_chat_profile_center_memory" --vpn-overlay "$tmp_linux_chat_profile_vpn_memory" --cache-dir "$tmp_linux_chat_core_cache" --max-facts 4 --format json)"
jq -e '.mode == "llmwave-big-chat-core-ask" and .verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .cache_status.cache_fresh == true and .grounded_packet.answer_allowed == true and .grounded_packet.readout_source == "compiled_chat_core_hot"' <<<"$big_linux_chat_core_auto_rebuild_json" >/dev/null
printf '\n# strict stale check\n' >>"$tmp_linux_chat_profile_center_memory"
big_linux_chat_core_stale_json="$("$llmwave_big" chat-core-ask --profile examples/linux-chat-core.profile.json --text "which package provides command bash" --residual-pack "$tmp_linux_chat_residual" --dialogue-overlay "$tmp_linux_chat_profile_memory" --centers-overlay "$tmp_linux_chat_profile_center_memory" --vpn-overlay "$tmp_linux_chat_profile_vpn_memory" --cache-dir "$tmp_linux_chat_core_cache" --max-facts 4 --no-auto-rebuild --format json)"
jq -e '.mode == "llmwave-big-chat-core-ask" and .verdict == "LINUX_CHAT_CORE_CACHE_STALE" and .cache_status.cache_fresh == false and (.cache_status.stale_reasons | index("source_memory_hash_changed")) and .grounded_packet.answer_allowed == false and .grounded_packet.decision_state == "CACHE_STALE_NO_AUTHORITY"' <<<"$big_linux_chat_core_stale_json" >/dev/null

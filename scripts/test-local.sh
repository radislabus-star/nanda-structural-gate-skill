...
jq -e '.mode == "llmwave-big-linux-chat-core-build" and .verdict == "LINUX_CHAT_CORE_CACHE_READY_NOT_GENERAL_LLM" and .manifest.profile_id == "linux-chat-core" and .manifest.cache_is_source_of_truth == false and .source_status.source_memory_loaded == true and .source_status.overlays_present == 3 and .claim_boundary.cache_is_source_of_truth == false and .claim_boundary.general_llm_ready == false' <<<"$big_linux_chat_core_build_json" >/dev/null

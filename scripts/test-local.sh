...
jq -e '.mode == "llmwave-big-linux-chat-core-ask" and .verdict == "LINUX_CHAT_CORE_PACKET_READY_NOT_GENERAL_LLM" and .cache_status.cache_fresh == true and .grounded_packet.cache_fresh == true and .grounded_packet.answer_allowed == true and .grounded_packet.readout_source == "compiled_chat_core_index" and .grounded_packet.decision_state == "ANSWER_GROUNDED" and (.grounded_packet.answer | ascii_downcase | contains("bash")) and .token_economics.grounded_packet_estimated_tokens > 0 and .token_economics.estimated_tokens_saved_vs_source > 0 and .token_economics.estimated_tokens_saved_vs_cache_index > 0 and .claim_boundary.cache_is_source_of_truth == false' <<<"$big_linux_chat_core_ask_json" >/dev/null
printf '\n# stale check\n' >>"$tmp_linux_chat_profile_center_memory"

...
jq -e '.mode == "llmwave-big-broad-dataset-doctor" and .verdict == "BROAD_DATASET_STRONG" and .quality.fact_count == 96 and .quality.route_count == 32 and .quality.domain_count == 8 and .quality.route_balance == 1 and .quality.duplicate_pressure == 0 and .gates.strong == true and .claim_boundary.external_broad_corpus_ready == true and .claim_boundary.proves_general_llm == false' <<<"$big_broad_public_doctor_json" >/dev/null
...
jq -e '.mode == "llmwave-big-broad-heldout-build" and .verdict == "BROAD_HELDOUT_SUITE_READY" and .metrics.generated_case_count == 16 and .metrics.withheld_fact_count == 16 and .metrics.covered_routes == 8 and .claim_boundary.heldout_suite_ready == true' <<<"$big_broad_heldout_json" >/dev/null

...
jq -e '.mode == "llmwave-memory-density" and .version == "v127-density-reality-check" and .claims_boundary.nonlinear_density_proven == false and .claims_boundary.lexical_baseline_compared == true and .rows[0].records == 16 and .rows[0].state == "DENSITY_STABLE" and .rows[0].wins_over_lexical_baseline == true and .rows[0].lexical_baseline.reversed_false_positive == true and .rows[2].records == 256 and .rows[2].focus_state == "HOT_FOCUS_READY" and .rows[].accuracy == 1' <<<"$memory_density_json" >/dev/null

...
run_chatcore_suite() {
  ...
  rm -f "$tmp_domain_draft"
  jq -e '.mode == "llmwave-big-chat-core-domain-gate" and .verdict == "DOMAIN_PACK_DRAFT_WATCH_NO_AUTHORITY" and .domain_id == "physics_material_layer" and .maturity == "candidate" and .safe_to_answer == false and .safe_to_learn_without_profile == false and .cache_mutated == false and .overlay_written == false and .authority_rights.answer_allowed == false and .authority_rights.overlay_write_allowed == false and .authority_rights.cache_mutation_allowed == false' <<<"$chat_core_domain_gate_json" >/dev/null
